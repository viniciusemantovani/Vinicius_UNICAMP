//Explicit Euler Method Code, by: Vinicius Esperanca Mantovani

#include <stdlib.h>
#include <stdio.h>

//Defined values to be used:
#define f(x,y) (1 - 2*y) //Define the function wanted here! In our case: a) 1 - 2y
...
struct eulerVectors *eE(float X0, float Y0, float h, float n){
    struct eulerVectors *vectors = malloc(sizeof(struct eulerVectors));
    vectors->X[0] = X0;
    vectors->Y[0] = Y0;

    for(int i = 0; i <= n - 1; i++){
        vectors->Y[i + 1] = vectors->Y[i] + h*f(vectors->X[i], vectors->Y[i]);
...
int main(int argc, char *argv){
    ...
    struct eulerVectors *vectors = eE(X0, Y0, h, n);
